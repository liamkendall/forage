...
max.species.l <- data.frame(rbind(max.species,max.species),range.4=
                 c(rep("Max-Potential",nrow(max.species)),
...
species.draws.1.r <- all_social_mod%>%
  add_epred_draws(newdata = max.species.l,
                      re_formula = ~(1|species)+(1|genus),
                      allow_new_levels=T)

species.draws.2.r <- all_social_mod%>%
  add_epred_draws(newdata = max.species.l,
                      re_formula = ~(1|genus),
                      allow_new_levels=T)

species.draws.3.r <- all_social_mod%>%
  add_epred_draws(newdata = max.species.l,
                      re_formula = NA,
                      allow_new_levels=T)

species.1.qi <- median_hdci(species.draws.1.r,.value=exp(.epred))%>%
...
typ.all.sp.p <- ggplot(species.values.typ.p,
  ...
  geom_jitter(data=forage.traits.plot2%>%
               ...
               filter(range.4%in%"Potential typical"),
             aes(x=distance/1000,
...
typ.all.sp.r <- ggplot(species.values.typ%>%
                         filter(range.4%in%"Typ-Realised"),aes(y=reorder(species,itd),
                                                           col=model.set.r))+
  geom_errorbarh(
    aes(xmin=.lower,
        xmax=.upper,
    ...
    inherit.aes = T,
    position = pd,
    height=0.1)+scale_x_log10()+
  geom_jitter(data=forage.traits.plot2%>%
                #filter(range.type%in%"Typical")%>%
                filter(range.4%in%"Realized typical"),
              aes(x=distance/1000,
                  y=reorder(species,itd),
                  col=range.type,
  ...
  geom_point(aes(x=.value,
             ...
             size=2.5,col="black",
             position = pd)+
  xlab("Realized foraging range (km)")+
  ylab("Taxa")+
  labs(name="Metric type",
       fill="Metric type",
       col="Metric type")+
...
max.all.sp.p <- ggplot(species.values.max2,aes(y=reorder(species,itd),
  ...
  scale_x_log10()+
  geom_jitter(data=forage.traits.plot2%>%
               filter(range.4%in%"Potential maximum"),
             aes(x=distance/1000,
                 y=reorder(species,itd),
                 col=range.type,
                 fill=range.type),alpha=0.75,
...
max.all.sp.r <- ggplot(species.values.max%>%
  ...
  geom_jitter(data=forage.traits.plot2%>%
               #filter(range.type%in%"Maximum")%>%
                filter(range.4%in%"Realized maximum"),
              aes(x=distance/1000,
                  y=reorder(species,itd),
                  col=range.type,
                  fill=range.type),alpha=0.75,
...
ggsave(sp.typ.plots,file="plots/Figure S3A final.jpg",
       device = "jpg", 
       width = 210, 
       height = 297, 
       units = "mm",
       dpi=600)
ggsave(sp.typ.plots,file="plots/Figure S3A final.pdf",
       device = cairo_pdf, 
       width = 210, 
       height = 297, 
       units = "mm",
...
ggsave(sp.max.plots,file="plots/Figure S3B final.jpg",
       device = "jpg", 
       width = 210, 
       height = 297, 
       units = "mm",
...
ggsave(sp.max.plots,file="plots/Figure S3B final.pdf",
       device = cairo_pdf, 
       width = 210, 
       height = 297, 
       units = "mm",
