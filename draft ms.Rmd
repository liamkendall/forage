---
title: Bee foraging distances can be predicted using body mass and level of eusociality
...
IB: Yes, names are important. Functional may be too ambigous. Potential vs realized can be an option.
...
IB: Yes, a negative result is still a very imteresting result!
...
Paragraph One: Ecological significance of foraging in terms of i) bee ecology and conservation, ii) ecosystem function and plant pollination.
Effects on foraging - landscape context etc
-links to body size

Types of measurements
Structure that feeder and homing as physiological measures - others as functional. Yes, this needs carefull wording to appeal a brad readership. I thnik I vote potential vs realized.

Why should this be revisited? 
-Many more studies (retrieve a true distribution of foraging ranges across species)
-new analytical methods
-need to incorporate things like phylogeny, traits, etc which explain the distribution of foraginhg ranges observed.
...
We checked the validity of species' names using (Zach to fill X, Y, and Z). We then collected the following traits for each species: female body size (measured as the intertegular distance, Cane et al. 1987, Kendall et al 2019), level of eusociality and floral specialisation. [IB: I think a statement on why this traits are selected is needed]()
...
For eusocial species, where applicable, we obtained ITD measurements for the caste used within the study (i.e. workers or queens[we mention queens were excluded above]()).
...
Oligolectic speies were defined as those species who consistently collect pollen from a single plant species or group of similar or related plant species, even in the presene of other pollen sources. 
...
We used Bayesian generalized linear mixed effect models to analyze bee foraging distances as a function of body size and functional traits. We chose to use an un-weighted model as the vast majority of studies did not provide variance components for typical foraging distances. Further, the nature of maximum foraging distance (i.e. the largest distance observed within a species) is incompatible with a meta-analytical model as it lacks a definable variance. [IB: I would be clear we are doing synthesis, not meta-analysis, those are different approaches]()
We modeled typical and maximum foraging distance separately. Observed foraging distances were excluded from analyses, as were all estimates with unknown metholodogy.[IB: ok, but then we need to use them for something, like e.g show the variation in foraging distances across species. I envision people would like to cite bees typically forage between X and X m from home]()
As is recommended in allometric studies, we log-transformed foraging distance prior to analyses. However, as intertegular distance is logarithmically related to body size (Cane et al. 1987; Kendall et al. 2019), we compared the predictive accuracy (R^2^) of models with and without log-transformed ITD. The raw ITD was considerably more predictive than ln(LTD), thus we present those models.[IB: I would go directly with untransformed ITD on the basis that is log related to body size]()
...
In addition, we also modelled distance ~ ITD * Lecty for solitary bees species. [IB: I would test Distance ~ ITD:Metric type + ITD:eusociality + ITD:lecticity in a single model]()
...
[Start with a descriptive histogram? Or three colored by type of metric?]()
...
```
Overall, body size was positively related to both maximum (*Marginal R^2^* = `r round(model.r2s[1,2][[1]],2)`) and typical (*Marginal R^2^* = `r round(model.r2s[9,2][[1]],2)`) foraging distances (Figure 1).  [IB: I wonder if marginal R2 is doing us here a bad favour, taking into account conditional effects include things like specie and phylo geny, for which we do have the data and can be used to predict. Estimates looks low]()
